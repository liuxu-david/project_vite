<script setup>
  ...
  import { uploadChunk } from "@/service/upload.js";
  ...
  const handleUploadChunk = async (chunk) => {
    return new Promise(async (resolve, reject) => {
      try {
        ...
        const result = await uploadChunk(fd);
        console.log(result);
        resolve(result);
      } catch (err) {
        reject(err);
  ...
  // 多个分片上传
  const handleUploadChunks = async (chunks, maxRequest = 2) => {
    // 控制并发上传数目
    ...
    return new Promise(async (resolve, reject) => {
      ...
      const handleConcurrent = async () => {
        if (requestStartIndex >= chunks.length - 1) {
  ...
  const handleUpload = async (event) => {
    ...
    // 上传切片
    await handleUploadChunks(chunks);
  };
</script>
